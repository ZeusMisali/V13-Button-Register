...
setInterval(async () => {  
  ...
  finishedPenals.forEach(async (x) => {
    ...
    if (x.type === "CHAT-MUTE") {
      ...
      client.channels.cache.get(conf.cmuteLogChannel).send({ embeds: [new MessageEmbed().setColor("#2f3136").setDescription(`${member.toString()} adlı Kullanıcının **Chat kanallarında** mute süresi doldu`)]});
    }
    if (x.type === "TEMP-JAIL") {
      x.active = false;
      await x.save();
    ...
    if (x.type === "VOICE-MUTE") {
      ...
      client.channels.cache.get(conf.vmuteLogChannel).send({ embeds: [new MessageEmbed().setColor("#2f3136").setDescription(`${member.toString()} adlı Kullanıcının **Ses Kanallarında** mute süresi doldu`)]});
    }
  });
