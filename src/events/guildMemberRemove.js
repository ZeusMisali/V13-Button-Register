...
module.exports = async (member) => {
  ...
  if (!inviteMemberData) {
    channel.wsend({ content: `\`${member.user.tag}\` adlı üye sunucudan çıktı ama kim tarafından davet edildiğini bulamadım.`});
  } else {
    ...
    channel.wsend({ content:`\`${member.user.tag}\` adlı üye sunucudan çıktı. ${inviter.tag} tarafından davet edilmişti. (**${total}** davet)`});
